...
`--skip-auth`, `--skip-install`, `--skip-skills`, and `--agent <name>`
are also available. See `firecrawl init --help`.
...
Store the API key in the project's environment (for example a local
`.env` file that is git-ignored), never committed to the repository:
